...
const DEFAULT_CLIENT_VERSION = "1.1.26";
...
export function loadAntigravityDirectConfigFromEnv(env: NodeJS.ProcessEnv = process.env): AntigravityDirectConfig {
  ...
  return {
    ...
    userAgent: env.ANTIGRAVITY_ADAPTER_USER_AGENT ?? `antigravity/hub/${clientVersion} (aidev_client; os_type=linux; arch=x64; cl=963137146)`,
  };
}
...
function normalizeTools(tools: unknown): Array<{ functionDeclarations: Record<string, unknown>[] }> | undefined {
  ...
  const declarations = tools.map((entry, index) => {
    ...
    const parameters = fn.parameters === undefined ? { type: "object", properties: {} } : sanitizeSchema(resolveRefs(fn.parameters));
    return {
      name: fn.name,
...
function requestEnvelope(request: OpenAIChatRequest, wireModel: string, projectId: string, opts: { signatureLookup?: SignatureLookup; proThinkingBudget?: number } = {}): Record<string, unknown> {
  ...
  // Mirror the captured Antigravity client for the Pro agent deployment: it
  // always sends the deployment cap, while the gateway enforces any smaller
  // caller limit from terminal usage. The high alias itself is not a working
  // direct deployment (normalizeWireModel maps it to gemini-pro-agent).
  if (wireModel === "gemini-pro-agent") {
    // Pro High is a thinking deployment: send a model-safe output cap and the
    // thinking budget, then let the gateway enforce any smaller caller limit from
    // terminal usage. Sending the tiny caller value directly would starve the
    // reasoning budget and produce an invalid provider request.
    generationConfig.maxOutputTokens = 65535;
    if (opts.proThinkingBudget && opts.proThinkingBudget > 0) generationConfig.thinkingConfig = { thinkingBudget: opts.proThinkingBudget };
  ...
  // Stable per-session trajectory: derive deterministically from the session id
  // so a multi-step Pi loop keeps one trajectory instead of a fresh id per HTTP
  // request. (Monotonic step index / last_execution_id / per-model model_enum
  // still need the wire reference and are NOT fabricated here.)
  const sessionId = stableSessionId(request.messages);
  const trajectoryId = createHash("sha256").update(`trajectory:${sessionId}`).digest("hex").slice(0, 32);
  const req: Record<string, unknown> = {
    ...
    labels: {
      ...
      ...(wireModel === "gemini-pro-agent" ? { model_enum: "MODEL_PLACEHOLDER_M16" } : {}),
    },
  };
  return {
    project: projectId,
...
export function createAntigravityDirectAdapter(config: AntigravityDirectConfig, deps: AntigravityDirectDependencies = {}) {
  ...
  const callInference = async (wireModel: string, request: OpenAIChatRequest, signal: AbortSignal): Promise<Response> => {
    ...
    const payload = requestEnvelope(request, wireModel, project, { signatureLookup, proThinkingBudget: config.proThinkingBudget });
        ...
        for await (const chunk of readSseJson(upstream.body, config.maxResponseBytes, controller.signal, config.idleTimeoutMs)) {
          ...
          for (const part of candidate?.content?.parts ?? []) {
            ...
            if (part.functionCall?.name) {
              ...
              const tool: ProviderToolCall = { id, name: part.functionCall.name, args: part.functionCall.args ?? {}, ...(part.thoughtSignature ? { thoughtSignature: part.thoughtSignature } : {}) };
              if (part.thoughtSignature) rememberSignature(id, part.thoughtSignature);
